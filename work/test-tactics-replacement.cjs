...
for(let cell=20;cell<=24;cell++)assert.equal(vm.runInContext(`v55PositionForCell(${cell})`,context),'mid','Reihe 5 gehört vollständig zum Mittelfeld');
assert.equal(vm.runInContext('v55PositionForCell(25)',context),'def');
assert.equal(vm.runInContext('v55PositionForCell(34)',context),'def');
assert.match(positionSource,/\.v55-zone-def\{top:71\.4286%\}/,'die sichtbare Abwehrzone beginnt erst nach Reihe 5');
