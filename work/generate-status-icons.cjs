...
function barDrawing(state){
 return '<rect width="92" height="11" rx="5.5" fill="#415558"/>'+
  `<rect x="1.5" y="1.5" width="${(89*state.percent/100).toFixed(1)}" height="8" rx="4" fill="#A3E4D4"/>`+
...
for(const state of freshnessStates)fs.writeFileSync(path.join(iconDir,`freshness-${state.id}.svg`),iconSvg(92,11,`Frische: ${state.label}`,barDrawing(state))+'\n');
...
const bars=Object.fromEntries(freshnessStates.map(state=>[state.id,dataUri(iconSvg(92,11,`Frische: ${state.label}`,barDrawing(state)))]));
...
const previewRows=formStates.map((state,index)=>{
 ...
 return `<g transform="translate(36 ${y})">${faceDrawing(state)}</g>`+
  ...
  `<g transform="translate(268 ${y+13})">${barDrawing(freshnessStates[index])}</g>`+
