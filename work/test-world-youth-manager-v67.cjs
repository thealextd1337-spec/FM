...
const talents=career.world.clubs.flatMap(club=>Array.from({length:20},(_,slot)=>call('v66Skill',call('v67Youth',career,club,3,slot)))).sort((a,b)=>a-b),medianTalent=talents[Math.floor(talents.length/2)];
assert(talents.at(-1)-medianTalent>=3,'wenige Spitzenkandidaten heben sich vom typischen Nachwuchs ab');
assert(talents.filter(value=>value>=medianTalent+2.5).length<talents.length*.15,'sehr gute Talente bleiben selten');
const releasedCareer=call('v61CreateCareer','GER-2','released-youth-seed'),releasedClub=call('v66Club',releasedCareer,'GER-2'),released=releasedClub.youthPool[0];
call('v67ReleaseYouth',releasedCareer,releasedClub.id,released.pid);
assert(!releasedClub.youthPool.includes(released)&&releasedCareer.world.market.freePlayers.includes(released),'entlassene Jugendspieler wechseln ablösefrei auf den Markt');
assert.strictEqual(released.youthReleasedSeason,1);
assert.strictEqual(call('v61ValidateCareer',releasedCareer),true);
call('v67SeasonEnd',releasedCareer);
assert(releasedCareer.world.market.freePlayers.includes(released),'entlassene Spieler bleiben in ihrer ersten Saison verfügbar');
releasedCareer.world.season=2;releasedCareer.world.transition=null;
call('v67SeasonEnd',releasedCareer);
assert(!releasedCareer.world.market.freePlayers.includes(released),'ohne Vertrag endet die Laufbahn nach der zweiten Saison');
assert(releasedCareer.world.retirements.some(item=>item.pid===released.pid&&item.season===2));
...
assert(call('v67TransitionHTML',career).includes('Kaderübersicht'));
...
assert(call('v67TransitionHTML',career).includes('Saisonübersicht')&&call('v67TransitionHTML',career).includes('Torschützen')&&call('v67TransitionHTML',career).includes('Pokalsieger'));
