...
let penaltyScene=null;
originalQuery('#match-area .v42-pitch-stage').append=node=>{penaltyScene=node;node.remove=()=>{penaltyScene=null}};
context.document.querySelector=selector=>selector==='#v50-penalty-scene'?penaltyScene:originalQuery(selector);
...
assert.equal(vm.runInContext('match.setPiece.spot.x',context),.049);
...
assert(penaltyScene,'the penalty scene is visible before the kick');
assert.equal(originalQuery('#match-overlay').hidden,true,'the generic banner must not overlap the penalty scene');
vm.runInContext('step(0,2)',context);
...
assert(penaltyScene,'the penalty result scene remains visible until resolved');
assert.equal(originalQuery('#match-overlay').hidden,true,'the result scene has no second banner');
vm.runInContext('step(0,2)',context);
assert.equal(vm.runInContext('match.setPiece.phase',context),'result','Elfmeteranzeige bleibt eine Sekunde länger sichtbar');
assert(penaltyScene,'Jubel ist nach zwei Sekunden noch sichtbar');
vm.runInContext('step(0,.16)',context);
...
assert.equal(vm.runInContext('match.setPiece',context),null);
assert.equal(penaltyScene,null,'the penalty scene closes before the goal banner');
assert.equal(originalQuery('#match-overlay').hidden,false,'the goal banner appears after the penalty scene');
...
assert.equal(vm.runInContext('match.setPiece.spot.y',parry),.964);
assert.equal(vm.runInContext('match.setPiece.spot.x',parry),.951);
