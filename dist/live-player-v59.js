...
function v59LivePlayerHTML(player){
 ...
 const scouting=player.t===0?`<section class="v59-player-scout"><h3>Bekannte Scoutingwerte</h3>${scoutingSkillsHTML(player).replace('class="scouting-skills"','class="scouting-skills v59-scout-grid"')}</section>`:'';
 return `<div class="v47-player-dialog-head"><div><h2 id="v47-player-title">${flagSVG(player.nation||'AT')} #${player.n} ${escapeHTML(player.name)}</h2><p>${escapeHTML(teamName)} · Live im Spiel</p></div><button type="button" class="v47-player-close" aria-label="Spielerinformationen schließen">×</button></div>${scouting}<div class="v47-player-stats">${rows.map(([label,value])=>`<span>${label}</span><b>${escapeHTML(String(value))}</b>`).join('')}</div><p class="v59-player-detail">Das Spiel pausiert, solange diese Ansicht geöffnet ist.</p><div class="v59-player-actions"><button type="button" class="v59-player-resume">Spiel fortsetzen</button></div>`;
}
