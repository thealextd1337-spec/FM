...
function v59LivePlayerHTML(player){
 ...
 const role=player.keeper?'Torwart':({def:'Verteidiger',mid:'Mittelfeldspieler',att:'Angreifer'}[player.assignedLine||player.line]||'Spieler');
 ...
 const flag=player.nation?.length===3&&typeof v61FlagSVG==='function'?v61FlagSVG(player.nation):flagSVG(player.nation||'AT');
 return `<div class="v47-player-dialog-head"><div><h2 id="v47-player-title">${flag} #${player.n} ${escapeHTML(player.name)}</h2><p>${escapeHTML(teamName)} · Live im Spiel</p></div><button type="button" class="v47-player-close" aria-label="Spielerinformationen schließen">×</button></div>${scouting}<div class="v47-player-stats">${rows.map(([label,value])=>{const color=label==='Form'&&typeof v51FormColor==='function'?v51FormColor(form):label==='Müdigkeit'&&typeof v51FreshnessColor==='function'?v51FreshnessColor(fresh):null;return`<span>${label}</span><b${color?` style="color:${color}"`:''}>${escapeHTML(String(value))}</b>`}).join('')}</div><p class="v59-player-detail">Das Spiel pausiert, solange diese Ansicht geöffnet ist.</p><div class="v59-player-actions"><button type="button" class="v59-player-resume">Spiel fortsetzen</button></div>`;
}
