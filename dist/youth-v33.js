...
function youthPanelHTML(){
 ...
 const development=signed.length?'<div class="youth-progress">'+signed.map(player=>{const games=player.seasons?.find(item=>item.number===activeSave.seasonNumber)?.games||0;return`<div><b>${escapeHTML(player.name)}</b><span>${player.age} Jahre · ${games} Einsätze · ${player.age<24?'Entwicklung durch Einsätze':'Entwicklung abgeschlossen'}</span></div>`}).join('')+'</div>':'';
