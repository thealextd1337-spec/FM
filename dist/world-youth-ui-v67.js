...
v61WorldScreen.addEventListener('click',event=>{
 ...
 try{
  ...
  if(button.hasAttribute('data-v67-release')){const pid=button.dataset.v67Release,player=v66Own(career).youthPool.find(item=>item.pid===pid);if(!player)return;const question=`${player.name} aus dem Nachwuchspool entlassen? Er ist diese und die nächste Saison für alle Vereine ablösefrei. Ohne Vertrag beendet er danach seine Karriere.`;if(!window.confirm(window.doppel6Language?.localize(question)||question))return;v67RunBusy('Nachwuchsentlassung wird gespeichert …',async()=>{v67ReleaseYouth(career,career.manager.managedClubId,pid);await v64UiSave();v61RenderCareer(career)});return}
