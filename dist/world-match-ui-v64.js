...
function v64UiPrematchPitch(career,fixture,state,side,options={}){
 ...
 return`<div class="pitch v64-prematch-field" style="--v64-shirt:${escapeHTML(colors[0])};--v64-shirt-edge:${escapeHTML(colors[1])}" aria-label="${options.label||'Deine Startelf auf dem Spielfeld'}"><div class="markings" aria-hidden="true"><div class="center-circle"></div><div class="half-line"></div><div class="box top"></div><div class="box bottom"></div></div><div class="attack-label">ANGRIFF ↑</div><div class="v64-zone-guide" aria-hidden="true"><div class="v64-zone-line v64-zone-mid"><b>MITTELFELD</b></div><div class="v64-zone-line v64-zone-def"><b>VERTEIDIGUNG</b></div></div><div class="grid v64-raster">${grid}</div>${keeper?`<div class="keeper"><button type="button" class="v64-keeper-choice ${keeperSlot===selected?'chosen':''}" ${pickAttribute}="${keeperSlot}" aria-label="${escapeHTML(keeper.name)}, Torwart, Form ${formText(v51EffectiveForm(keeper))}, ${freshText(keeper.fresh)}" aria-pressed="${keeperSlot===selected}"><span class="token">${escapeHTML(keeper.n)}</span>${v51PitchFaceHTML(keeper)}${v51PitchBarHTML(keeper)}<span class="position-label">TW</span><span class="player-label" data-v64-pitch-profile="${escapeHTML(keeper.pid)}" role="link" tabindex="0" aria-label="Profil von ${escapeHTML(keeper.name)} öffnen">${v61FlagSVG(keeper.nation)}<span>${escapeHTML(keeper.name.toUpperCase())}</span></span>${Number.isFinite(keeperRating)?`<span class="v64-live-rating" aria-label="Aktuelle Note ${keeperRating.toFixed(1).replace('.',',')}">${keeperRating.toFixed(1).replace('.',',')}</span>`:''}</button></div>`:''}</div>`;
}
...
function v64UiPrematchBench(career,fixture,state,side){
 ...
 return`<section class="v64-bench-section compact-bench"><div class="compact-bench-head"><h3>Ersatzbank</h3><button type="button" class="v64-bench-undo" data-v64-undo ${v64UiUndo?'':'disabled'} aria-label="Letzte Änderung zurücknehmen" title="Letzte Änderung zurücknehmen">↶ Rückgängig</button><span>${plan.bench.length} / 5 · Auf ein Trikot ziehen</span></div><div class="bench-strip v64-bench-strip">${plan.bench.map(pid=>{const item=roster.find(player=>player.pid===pid);return`<article class="bench-chip v64-bench-chip" draggable="true" data-v64-bench-card="${escapeHTML(pid)}"><i class="fitness-dot ${item.fresh<52?'tired':item.fresh<72?'ready':'fresh'}"></i><div class="bench-select" data-v64-bench="${escapeHTML(pid)}"><span class="bench-title">${v61FlagSVG(item.nation)}<b>#${escapeHTML(item.n)} ${escapeHTML(item.name)}</b></span><span class="bench-meta">${v61PositionNames[item.line]} · ${freshText(item.fresh)}</span>${v51StatusHTML(item,item.fresh)}<span class="bench-skills">${v55TopSkillsHTML(item)}</span></div><button type="button" class="player-link" data-v64-profile="${escapeHTML(pid)}">Details</button></article>`}).join('')}</div><div class="compact-actions"><button type="button" data-v64-quick="fresh">Frischeste</button><button type="button" data-v64-quick="defensive">Defensiver</button><button type="button" data-v64-quick="offensive">Offensiver</button></div><p class="lineup-status" role="status">Aufstellung gültig · Änderungen werden automatisch gespeichert.</p></section>`;
}
...
function v64UiSwapSlots(career,fixture,state,side,first,second){
 const plan=side===0?fixture.plan.home:fixture.plan.away,a=plan.starters[first],b=plan.starters[second];
 if(!a||!b||a===b)return;
 ...
 if(state.roles[a]==='gk')return;
 v64MoveCell(career,fixture,state,side,a,v64EnsureCells(state,side)[b]);
}
...
function v64UiClick(event){
 const profile=event.target.closest?.('[data-v64-pitch-profile]');
 if(profile&&v61CurrentCareer?.world.activeMatch){event.stopPropagation();v61OpenProfile(profile.dataset.v64PitchProfile,profile);return}
 ...
 try{
  ...
  if(button.dataset.v64Cell!==undefined&&state.phase==='prematch'){
   const side=v64UiOwnSide(fixture),pid=(side===0?fixture.plan.home:fixture.plan.away).starters[v64SelectedSlot];
   v64UiRemember(fixture,state);if(v64MoveCell(career,fixture,state,side,pid,Number(button.dataset.v64Cell))){v64UiSave();v64UiRender(career)}return;
  }
...
function v64UiDragTarget(event){return event.target.closest?.('[data-v64-cell],[data-v64-pick-slot],[data-v64-bench-card]')}
function v64UiCanDrop(target){
 ...
 if(target.dataset.v64Cell!==undefined)return v64UiDrag.kind==='field'?state.roles[source]!=='gk':Boolean(dest&&v64Player(career,fixture,side,source).keeper===v64Player(career,fixture,side,dest).keeper);
...
function v64UiDragStart(event){
 ...
 const target=v64UiDragTarget(event);if(!target||target.dataset.v64PickSlot===undefined&&target.dataset.v64BenchCard===undefined)return;
...
function v64UiDrop(event){
 ...
 event.preventDefault();v64UiDropAction(v64UiDrag,target);
}
function v64UiDropAction(source,target){
 const career=v61CurrentCareer,fixture=v64ActiveFixture(career),state=career.world.activeMatch.state,side=v64UiOwnSide(fixture),plan=side===0?fixture.plan.home:fixture.plan.away;
 try{
  v64UiRemember(fixture,state);
  if(source.kind==='field'&&target.dataset.v64Cell!==undefined)v64MoveCell(career,fixture,state,side,plan.starters[source.slot],Number(target.dataset.v64Cell));
  else if(source.kind==='field'&&target.dataset.v64PickSlot!==undefined)v64UiSwapSlots(career,fixture,state,side,source.slot,Number(target.dataset.v64PickSlot));
  else if(source.kind==='bench'&&target.dataset.v64PickSlot!==undefined)v64SetPrematchSlot(career,fixture,state,side,Number(target.dataset.v64PickSlot),source.pid);
...
v61WorldScreen.addEventListener('keydown',event=>{if((event.key==='Enter'||event.key===' ')&&event.target.matches('[data-v64-pitch-profile]')){event.preventDefault();event.stopPropagation();v61OpenProfile(event.target.dataset.v64PitchProfile,event.target)}});
