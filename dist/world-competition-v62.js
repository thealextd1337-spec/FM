...
function v62CareerViewsHTML(career){
 ...
 const overview=`<section class="v62-season"><div class="v62-season-head"><div><p class="eyebrow">Weltkalender</p><h2>Saison ${career.world.season}</h2><p>${nextText}</p></div></div><p class="v62-explainer">Deine Partien laufen auf dem Spielfeld; andere Begegnungen werden mit denselben Matchregeln kompakt simuliert. Die Vereinswirtschaft folgt.</p>${recent.length?`<h3>Zuletzt in deiner Liga und im Pokal</h3><div class="v62-recent">${recent.map(fixture=>v62ResultHTML(career,fixture)).join('')}</div>`:''}<div class="v62-competitions"><details open><summary>Meine Liga · ${v61CountryNames[ownCountry]}</summary>${v62TableHTML(career,ownLeague)}</details></div></section>`;
 const competition=`<section class="v62-season"><h2 class="v46-view-heading">Wettbewerbe · Saison ${career.world.season}</h2><div class="v62-competitions"><details open><summary>Meine Liga · ${v61CountryNames[ownCountry]}</summary>${v62TableHTML(career,ownLeague)}${ownLeague.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details><details><summary>Europacup</summary>${v62TableHTML(career,europe)}${europe.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details>${v61Countries.map(([country,name])=>{const cup=competitions.find(item=>item.type==='cup'&&item.country===country),league=competitions.find(item=>item.type==='league'&&item.country===country);return`<details><summary>${name} · Liga und Pokal</summary>${v62TableHTML(career,league)}<h4>Nationaler Pokal</h4>${cup.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details>`}).join('')}</div></section>`;
