...
function v62CupBracketHTML(career,cup){
 ...
 return`<div class="v62-cup-bracket-scroll" role="region" tabindex="0" aria-label="Turnierbaum ${escapeHTML(v61CountryNames[cup.country])} · Saison ${cup.season}"><div class="v62-cup-bracket">${rounds.map(([code,label,count])=>{const games=cup.fixtures.filter(item=>item.round===code).sort((a,b)=>(a.pair??0)-(b.pair??0)||a.id.localeCompare(b.id));return`<section class="v62-cup-round"><h4>${label}</h4><div class="v62-cup-round-games">${Array.from({length:count},(_,index)=>games[index]?v62ResultHTML(career,games[index]):`<div class="v62-cup-pending">Paarung folgt nach ${code==='SF'?'dem Viertelfinale':'dem Halbfinale'}</div>`).join('')}</div></section>`}).join('')}</div></div>`;
}
