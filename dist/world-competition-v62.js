...
function v62CareerViewsHTML(career){
 ...
 const overview=`<section class="v62-season"><div class="v62-season-head"><div><p class="eyebrow">Saisonüberblick</p><h2>Saison ${career.world.season}</h2><p>${nextText}</p></div></div>${v62NextOpponentHTML(career,next)}<h3>Deine letzten fünf Spiele</h3>${v62FormHTML(career,managed)}${ownRecent.length?`<div class="v62-recent v62-own-recent">${ownRecent.map(({fixture})=>v62ResultHTML(career,fixture)).join('')}</div>`:'<p class="v62-explainer">Noch kein Spiel absolviert.</p>'}<div class="v62-competitions"><details open><summary>${v62LeagueLabel(ownCountry)}</summary>${v62TableHTML(career,ownLeague)}</details></div></section>`;
