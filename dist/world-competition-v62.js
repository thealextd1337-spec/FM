...
function v62NextOpponentHTML(career,fixture){
 ...
 return`<section class="v49-match-preview v62-next-opponent"><p>Nächster Gegner · ${competition.type==='league'?v62LeagueLabel(competition.country):competition.type==='cup'?'Nationaler Pokal':'Europacup'} · ${v62Date(fixture.day)}</p><div class="v49-fixture">${side(fixture.homeId,'Heim')}<span>gegen</span>${side(fixture.awayId,'Auswärts')}</div>${duels.length?`<div class="v62-duels"><h4>Letzte Duelle</h4>${duels.map(({fixture:game,season})=>{const home=game.homeId===managed,ownGoals=home?game.result.homeGoals:game.result.awayGoals,otherGoals=home?game.result.awayGoals:game.result.homeGoals,outcome=ownGoals>otherGoals?'Sieg':ownGoals<otherGoals?'Niederlage':'Remis';return`<div><small>Saison ${season} · ${v62Date(game.day)} · ${home?'Heim':'Auswärts'}</small><strong>${outcome} <b>${ownGoals}:${otherGoals}</b></strong></div>`}).join('')}</div>`:''}</section>`;
}
...
function v62CareerViewsHTML(career){
 ...
 const ownCompetitionPanel=`<div id="v62-own-competitions" data-v62-competition-area="own" role="tabpanel" aria-labelledby="v62-tab-own" ${v62CompetitionArea==='own'?'':'hidden'}><h3>Deine Wettbewerbe</h3><div class="v62-competitions"><details open><summary>${v62LeagueLabel(ownCountry)}</summary>${v62TableHTML(career,shownLeague)}${shownLeague.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details><details open><summary>${v61FlagSVG(ownCountry)} ${escapeHTML(v61CountryNames[ownCountry])} · Nationaler Pokal</summary>${shownCup.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details><details open><summary>Europacup</summary>${v62EuropeDetailsHTML(career,shownEurope)}</details></div></div>`;
 const otherCompetitionPanel=`<div id="v62-other-competitions" class="v62-other-countries" data-v62-competition-area="other" role="tabpanel" aria-labelledby="v62-tab-other" ${v62CompetitionArea==='other'?'':'hidden'}><h3>Andere Länder</h3><label>Land auswählen <select data-v62-country-choice>${otherCountries.map(([country,name])=>`<option value="${country}" ${country===selectedCountry?'selected':''}>${escapeHTML(name)}</option>`).join('')}</select></label>${otherCountries.map(([country,name])=>{const league=shown.find(item=>item.type==='league'&&item.country===country),cup=shown.find(item=>item.type==='cup'&&item.country===country);return`<div class="v62-country-panel" data-v62-country-panel="${country}" ${country===selectedCountry?'':'hidden'}><details open><summary>${v62LeagueLabel(country)}</summary>${v62TableHTML(career,league)}${league.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details><details open><summary>${v61FlagSVG(country)} ${escapeHTML(name)} · Nationaler Pokal</summary>${cup.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details></div>`}).join('')}</div>`;
