...
function v62CareerViewsHTML(career){
 ...
 const recent=fixtures.filter(fixture=>{const competition=competitionById.get(fixture.competitionId);return fixture.result&&fixture.day<=career.world.calendarCursor&&(competition.country===ownCountry||competition.type==='europe'&&(fixture.homeId===managed||fixture.awayId===managed))}).sort((a,b)=>b.day-a.day||Number(b.homeId===managed||b.awayId===managed)-Number(a.homeId===managed||a.awayId===managed)).slice(0,8);
 const ownLeague=competitions.find(item=>item.type==='league'&&item.country===ownCountry);
 ...
 const overview=`<section class="v62-season"><div class="v62-season-head"><div><p class="eyebrow">Saisonüberblick</p><h2>Saison ${career.world.season}</h2><p>${nextText}</p></div></div><p class="v62-explainer">Deine Partien laufen auf dem Spielfeld; andere Begegnungen werden mit denselben Matchregeln kompakt simuliert. Die Vereinswirtschaft folgt.</p>${recent.length?`<h3>Zuletzt in deiner Liga und im Pokal</h3><div class="v62-recent">${recent.map(fixture=>v62ResultHTML(career,fixture)).join('')}</div>`:''}<div class="v62-competitions"><details open><summary>${v62LeagueLabel(ownCountry)}</summary>${v62TableHTML(career,ownLeague)}</details></div></section>`;
 const competition=`<section class="v62-season"><h2 class="v46-view-heading">Wettbewerbe · Saison ${career.world.season}</h2><div class="v62-competitions"><details open><summary>${v62LeagueLabel(ownCountry)}</summary>${v62TableHTML(career,ownLeague)}${ownLeague.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details><details open><summary>Europacup</summary>${v62TableHTML(career,europe)}${europe.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details>${v61Countries.map(([country,name])=>{const cup=competitions.find(item=>item.type==='cup'&&item.country===country),league=competitions.find(item=>item.type==='league'&&item.country===country);return`${country===ownCountry?'':`<details><summary>${v62LeagueLabel(country)}</summary>${v62TableHTML(career,league)}${league.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details>`}<details><summary>${v61FlagSVG(country)} ${escapeHTML(name)} · Nationaler Pokal</summary>${cup.fixtures.map(fixture=>v62ResultHTML(career,fixture)).join('')}</details>`}).join('')}</div></section>`;
 const stats=competitions.map(item=>{const games=item.fixtures.filter(fixture=>fixture.result&&(fixture.homeId===managed||fixture.awayId===managed));if(!games.length)return null;const wins=games.filter(fixture=>fixture.result.homeGoals!==fixture.result.awayGoals&&(fixture.homeId===managed?fixture.result.homeGoals>fixture.result.awayGoals:fixture.result.awayGoals>fixture.result.homeGoals)).length,draws=games.filter(fixture=>fixture.result.homeGoals===fixture.result.awayGoals).length,goals=games.reduce((sum,fixture)=>sum+(fixture.homeId===managed?fixture.result.homeGoals:fixture.result.awayGoals),0),label=item.type==='league'?'Liga':item.type==='cup'?'Nationaler Pokal':'Europacup';return`<div class="v62-stat-card"><strong>${label}</strong><span>${games.length} Spiele</span><span>${wins} Siege · ${draws} Remis</span><span>${goals} Tore</span></div>`}).filter(Boolean).join('');
 const statistics=`<section class="v62-season"><h2 class="v46-view-heading">Statistik · Saison ${career.world.season}</h2><p class="v62-explainer">Vereinsergebnisse aus abgeschlossenen Partien. Einsatzminuten und Tore der Spieler stehen in ihren Profilen.</p><div class="v62-stat-grid">${stats||'<p>Noch keine Partie gespielt.</p>'}</div></section>`;
