...
function v47ReportHTML(report,shootout){
 ...
 const matchAward=matchWinner?`<div class="v47-match-award">${typeof v62AwardIcon==='function'?v62AwardIcon(null,'man-of-the-match'):'★'}<span><small>Man of the Match</small><button type="button" class="v47-award-player" data-v47-award-profile="${escapeHTML(matchWinner.pid)}" aria-label="Profil von ${escapeHTML(matchWinner.name)} öffnen">${escapeHTML(matchWinner.name)}</button><em>${escapeHTML(matchWinner.team===0?report.ownName:report.opponentName)} · Note ${Number(matchWinner.stats.rating).toFixed(1).replace('.',',')}</em></span></div>`:'';
...
function v47ShowReport(report,shootout){
 ...
 v47Dialog.querySelectorAll('.v47-close,.v47-done,.v47-report-close').forEach(button=>button.onclick=()=>{v47PlayerDialog.close?.();v47Dialog.close()});
 v47Dialog.onclick=event=>{const profile=event.target.closest('[data-v47-award-profile]');if(profile&&typeof v61OpenProfile==='function'){v61OpenProfile(profile.dataset.v47AwardProfile,profile);return}const row=event.target.closest('[data-report-player]');if(row)v47OpenPlayerStats(report,Number(row.dataset.reportPlayer))};
