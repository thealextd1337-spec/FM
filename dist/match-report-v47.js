...
function v47Snapshot(opponentName){
 ...
 return{ownName,opponentName:opponentName||match.cupOpponentName||activeOpponent()?.name||'Gegner',competition:match.cup?{type:'cup',stage:match.cup.stage}:{type:'league',round:Math.max(0,activeSave.currentRound-1)},score:[...match.score],shots:[...match.shots],possession:[...match.possession],setPieceStats:match.setPieceStats?structuredClone(match.setPieceStats):null,players:match.people.map(player=>({name:player.name,n:player.n,nation:player.nation||'AT',keeper:Boolean(player.keeper),team:player.t,stats:{...player.stats}}))};
}
...
function v47PlayerRows(report,team){return report.players.filter(player=>player.team===team).map(player=>{const stats=player.stats,index=report.players.indexOf(player);return`<button type="button" class="v47-player" data-report-player="${index}" aria-label="Statistik von ${escapeHTML(player.name)} anzeigen"><span>${flagSVG(player.nation)}<em title="${escapeHTML(player.name)}">#${player.n} ${escapeHTML(player.name)}</em></span><b>${Number(stats.rating||0).toFixed(1).replace('.',',')}</b><b>${stats.goals||0}</b><b>${stats.assists||0}</b></button>`}).join('')}
function v47PlayerStatsHTML(player,teamName){
 ...
 else rows.push(['Schüsse',s.shots||0],['Davon aufs Tor',s.onTarget||0],['Zweikämpfe gewonnen',`${s.duelsWon||0} / ${s.duels||0}`],['Ballabfänge',s.interceptions||0]);
 return`<div class="v47-player-dialog-head"><div><h2 id="v47-player-title">${flagSVG(player.nation)} #${player.n} ${escapeHTML(player.name)}</h2><p>${escapeHTML(teamName)} · Statistik dieses Spiels</p></div><button type="button" class="v47-player-close" aria-label="Spielerstatistik schließen">×</button></div><div class="v47-player-stats">${rows.map(([label,value])=>`<span>${label}</span><b>${value}</b>`).join('')}</div>`;
}
...
function v47CompetitionHTML(report){
 const type=report.competition?.type||(match?.cup?'cup':'league'),cup=type==='cup',index=cup?(report.competition?.stage??match?.cup?.stage??0):(report.competition?.round??Math.max(0,activeSave.currentRound-1)),games=(cup?activeSave.cup?.rounds[index]:activeSave.schedule[index])||[],others=games.filter(game=>game.result&&game.home!=='user'&&game.away!=='user'),label=cup?`Pokal · ${v41Labels[index]}`:`Liga · Spieltag ${index+1}`;
 const results=`<section class="v47-competition-section"><h3>Weitere Ergebnisse</h3>${others.map(game=>v47OtherResultHTML(game,type)).join('')||'<p class="help">Keine weiteren Spiele in dieser Runde.</p>'}</section>`;
...
function v47ReportHTML(report,shootout){
 ...
 return`<div class="v47-body"><div class="v47-head"><div><h2 id="v47-report-title">Spielbericht</h2><p>Spielende · Noten von 1 bis 10 · T = Tore · V = Vorlagen</p></div><button type="button" class="v47-close" aria-label="Spielbericht schließen">×</button></div><div class="v47-score"><span class="${winner===0?'v47-winner':''}">${escapeHTML(report.ownName)}</span><strong>${report.score.join(' : ')}</strong><span class="${winner===1?'v47-winner':''}">${escapeHTML(report.opponentName)}</span></div>${shootout?`<p class="help">Elfmeterschießen: ${shootout.score.join(' : ')} · ${escapeHTML(shootout.winner===0?report.ownName:report.opponentName)} gewinnt</p>`:''}<div class="v47-stat-head"><span>Teamstatistik</span><b title="${escapeHTML(report.ownName)}">${escapeHTML(homeCode)}</b><b title="${escapeHTML(report.opponentName)}">${escapeHTML(awayCode)}</b></div>${rows.map(([label,home,away])=>`<div class="v47-stat-row"><span>${label}</span><b>${home}</b><b>${away}</b></div>`).join('')}<div class="v47-rosters">${[[0,report.ownName],[1,report.opponentName]].map(([team,name])=>`<section class="v47-roster"><h3>${escapeHTML(name)}</h3><div class="v47-player-head"><span>Spieler</span><span>Note</span><span>T</span><span>V</span></div>${v47PlayerRows(report,team)}</section>`).join('')}</div><div class="v47-actions"><button type="button" class="menu-action v47-done">Schließen</button><button type="button" class="v47-menu">Weiter</button></div></div>`;
}
