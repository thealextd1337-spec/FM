...
function v47PlayerRows(report,team){return report.players.filter(player=>player.team===team).map(player=>{const stats=player.stats,index=report.players.indexOf(player),color=typeof v51RatingColor==='function'?v51RatingColor(Number(stats.rating)):null;return`<button type="button" class="v47-player" data-report-player="${index}" aria-label="Statistik von ${escapeHTML(player.name)} anzeigen"><span>${flagSVG(player.nation)}<em title="${escapeHTML(player.name)}">#${player.n} ${escapeHTML(player.name)}</em></span><b${color?` style="color:${color}"`:''}>${Number(stats.rating||0).toFixed(1).replace('.',',')}</b><b>${stats.goals||0}</b><b>${stats.assists||0}</b></button>`}).join('')}
