...
function v25UpdateLiveBoard(){
 ...
 const home=ui.scoreboard.querySelector('.home-team'),away=ui.scoreboard.querySelector('.away-team'),homeName=activeSave?.club||'FC Viertel',awayName=activeSave?activeOpponent().name:'SC Hafen';home.innerHTML=`<b>${escapeHTML(homeName)}</b><small>${escapeHTML(v25Formation(0))}</small><div class="goal-list">${v25Scorers(0)}</div>`;away.innerHTML=`<b>${escapeHTML(awayName)}</b><small>${escapeHTML(v25Formation(1))}</small><div class="goal-list">${v25Scorers(1)}</div>`;
}
