...
v25Style.textContent+=`.keeper .player-link{font-size:9px;line-height:1.2;font-weight:700;letter-spacing:.5px}@media(max-width:760px){.keeper .player-link{font-size:8px}}`;
v25Style.textContent+=`.pitch-role-bar{margin:12px 0;padding:14px;border:1px solid #516b66;border-radius:9px;background:#172d30}.pitch-role-head b{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pitch-role-meta{margin:0 0 9px;color:#b9ccc6;font-size:11px}.pitch-role-skills{margin:0 0 11px;color:#a9c8be;font-size:10px;line-height:1.4}.pitch-role-actions{display:none}.pitch-role-close{display:none;min-width:32px;min-height:32px;border:0;background:transparent;color:#fff;font-size:22px}@media(max-width:760px){#pitch-role-bar{display:none;position:fixed;z-index:30;left:0;right:0;bottom:0;max-height:48vh;overflow:auto;margin:0;padding:11px 14px calc(11px + env(safe-area-inset-bottom));border-radius:12px 12px 0 0;box-shadow:0 -10px 32px #061413db}#pitch-role-bar.is-open:not([hidden]){display:block}#pitch-role-bar .pitch-role-close{display:inline-block}#pitch-role-bar .pitch-role-actions{display:flex;gap:8px;margin-top:9px}#pitch-role-bar .pitch-role-actions button{flex:1;min-height:42px;border:1px solid #55716c;border-radius:6px;background:#284442;color:#fff;font-weight:700}#pitch-role-bar .pitch-role-actions [data-v25-start]{background:var(--club-primary);color:var(--club-text)}#game-screen:has(#pitch-role-bar.is-open:not([hidden])){padding-bottom:195px}#game-screen:has(#pitch-role-bar.is-open:not([hidden])) #start{visibility:hidden}}`;

const v25PositionShort={gk:'TOR',def:'VER',mid:'MIT',att:'ANG'};
let v25RoleOpen=false;

function v25RoleBar(){
 const player=players[selected],anchor=$('#duration');if(!anchor||!player)return;document.querySelector('#pitch-role-bar')?.remove();
 const bar=document.createElement('section');bar.id='pitch-role-bar';bar.className=`pitch-role-bar${v25RoleOpen?' is-open':''}`;bar.hidden=v24Tab!=='lineup';
 const warning=v24PositionWarning(player),error=v24Validation()[0];
 bar.innerHTML=`<div class="pitch-role-head"><b>#${player.n} ${escapeHTML(player.name)} · ${v25PositionShort[player.line]}</b><button type="button" class="player-link" data-open-player="${escapeHTML(player.pid)}">Profil</button><button type="button" class="pitch-role-close" data-v25-close aria-label="Spielerauswahl schließen">×</button></div><p class="pitch-role-meta">${escapeHTML(freshText(player.fresh??100))} · ${escapeHTML(formText(player.form??0))}e Form</p><p class="pitch-role-skills">${escapeHTML(v24TopSkills(player))}</p><div class="segmented" aria-label="Ausrichtung von ${escapeHTML(player.name)}"><button data-pitch-role="-1" aria-pressed="${player.role===-1}" ${player.role===-1?'class="active"':''}>Defensiv</button><button data-pitch-role="0" aria-pressed="${player.role===0}" ${player.role===0?'class="active"':''}>Ausgewogen</button><button data-pitch-role="1" aria-pressed="${player.role===1}" ${player.role===1?'class="active"':''}>Offensiv</button></div>${warning?`<p class="role-warning">${escapeHTML(warning)}</p>`:''}<div class="pitch-role-actions"><button type="button" data-v25-close>Fertig</button><button type="button" data-v25-start ${error?'disabled':''}>Spiel starten</button></div>`;
 anchor.insertAdjacentElement('afterend',bar);
 bar.querySelectorAll('[data-pitch-role]').forEach(button=>button.onclick=()=>{if(players[selected].role===+button.dataset.pitchRole)return;v24Remember();players[selected].role=+button.dataset.pitchRole;saveCurrent();render()});
 bar.querySelectorAll('[data-v25-close]').forEach(button=>button.onclick=()=>{v25RoleOpen=false;bar.classList.remove('is-open')});
 bar.querySelector('[data-v25-start]').onclick=()=>{$('#start').click()};bindPlayerCardLinks(bar);
}
function v25SelectPitchPlayer(){v25RoleOpen=true;document.querySelector('#pitch-role-bar')?.classList.add('is-open')}
...
showTactics=function(){v25RoleOpen=false;v24ShowTacticsV25();v25Prematch();document.querySelectorAll('footer span:first-child').forEach(element=>element.textContent='Doppel 6 / PROTOTYP 39')};
const v24StartWithRoleBar=start;
start=function(){const result=v24StartWithRoleBar();if(running){const roleBar=$('#pitch-role-bar');if(roleBar)roleBar.hidden=true;v25RoleOpen=false}return result};$('#start').onclick=()=>start();
