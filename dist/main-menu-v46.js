...
const v46Tabs=[['overview','⌂','Übersicht'],['squad','♙','Kader'],['transfers','↔','Transfers'],['competition','🏆','Wettbewerbe'],['club','⚑','Verein']];
...
function v46ArrangeCenter(){
 ...
 const nav=document.createElement('nav');nav.className='v46-nav';nav.setAttribute('aria-label','Karrieremenü');nav.innerHTML=v46Tabs.map(([key,icon,label])=>`<button type="button" data-v46-tab="${key}"><span aria-hidden="true">${icon}</span>${label}</button>`).join('');menu.insertAdjacentElement('afterend',nav);
 for(const node of[...clubCenter.children])if(node!==intro&&node!==menu&&node!==nav&&node.tagName!=='FOOTER')node.dataset.v46View=v46TabFor(node);
