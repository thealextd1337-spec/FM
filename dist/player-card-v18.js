...
function playerCardHTML(player){
 ...
 return`<div class="player-card-head"><div>${flagSVG(player.nation)}<p class="eyebrow">${escapeHTML(nation)}</p><h2>${escapeHTML(player.name)}</h2><span>${escapeHTML(position)} · ${player.age} Jahre · ${escapeHTML(player.foot||'Rechts')}fuß</span></div><button id="close-player-card" aria-label="Spielerkarte schließen">×</button></div><div class="player-card-facts"><span>Status<b>${status}</b></span><span>Form<b>${formText(player.form||0)}</b></span><span>Fitness<b>${freshText(player.fresh??100)}</b></span><span>Spielertyp<b>${escapeHTML(player.type||position)}</b></span></div><section><h3>Fähigkeiten</h3>${scoutingSkillsHTML(player)}<small>Die Farben zeigen Fähigkeitsstufen. Exakte Werte bleiben verborgen.</small></section><section class="season-stats-section">${seasonStatsPagerHTML(player,'h3')}</section>`;
}
