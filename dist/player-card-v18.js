...
function playerStatsForSeason(player,number){return(player.seasons||[]).find(season=>season.number===number)||blankStats(number)}
function playerCardHTML(player){
 const current=playerStatsForSeason(player,activeSave.seasonNumber),previous=[...(player.seasons||[])].filter(season=>season.number<activeSave.seasonNumber).sort((a,b)=>b.number-a.number)[0],career=sumStats(player),position=player.keeper?'Torwart':lineNames[player.line],nation=nationData[player.nation]?.name||player.nation||'Unbekannt',starting=player.keeper||activeSave.lineup?.includes(player.n),status=activeSave.squad?.some(item=>item.pid===player.pid)||activeSave.keeper?.pid===player.pid?(starting?'Startaufstellung':'Ersatzbank'):'Beobachteter Spieler';
 return`<div class="player-card-head"><div>${flagSVG(player.nation)}<p class="eyebrow">${escapeHTML(nation)}</p><h2>${escapeHTML(player.name)}</h2><span>${escapeHTML(position)} · ${player.age} Jahre · ${escapeHTML(player.foot||'Rechts')}fuß</span></div><button id="close-player-card" aria-label="Spielerkarte schließen">×</button></div><div class="player-card-facts"><span>Status<b>${status}</b></span><span>Form<b>${formText(player.form||0)}</b></span><span>Fitness<b>${freshText(player.fresh??100)}</b></span><span>Spielertyp<b>${escapeHTML(player.type||position)}</b></span></div><section><h3>Scoutingbericht</h3><p>${escapeHTML(scoutingText(player))}</p>${scoutingSkillsHTML(player)}<small>Die Einschätzungen sind bewusst ungenau. Exakte Fähigkeitswerte bleiben verborgen.</small></section><section><h3>Aktuelle Saison</h3>${statBlock(player,current)}</section><section><h3>Saison davor</h3>${previous?`<p class="history-label">${escapeHTML(previous.label)}</p>${statBlock(player,previous)}`:'<p class="help">Noch keine abgeschlossene Saison.</p>'}</section><section><h3>Karriere</h3>${statBlock(player,career)}</section>`;
}
function openPlayerCard(pid){
 const player=findPlayerProfile(pid);if(!player)return false;playerCardDialog.innerHTML=playerCardHTML(player);playerCardDialog.querySelector('#close-player-card').onclick=()=>playerCardDialog.close?.();if(playerCardDialog.showModal)playerCardDialog.showModal();else playerCardDialog.setAttribute('open','');return true;
}
