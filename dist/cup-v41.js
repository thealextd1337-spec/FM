...
function v41FinalCeremony(){
 ...
 const shirt=(kit,player,side)=>`<span title="${escapeHTML(player.name)}" style="--shirt:${escapeHTML(kit.main)};--trim:${escapeHTML(kit.trim)};--text:${textColor(kit.main)};--face:${side===0?'8deg':'-8deg'}">${player.n}</span>`;
 const homeKit=currentKits().home,awayKit=opponent.kits.away;
 $('#start').hidden=true;
 $('#game-screen .intro').insertAdjacentHTML('afterend',`<section id="cup-ceremony" class="cup-ceremony"><p class="eyebrow">POKALFINALE · EINLAUF</p><h2>${escapeHTML(activeSave.club)} gegen ${escapeHTML(opponent.name)}</h2><div class="cup-ceremony-pitch"><div class="cup-ceremony-side left"><b>${escapeHTML(activeSave.club)}</b><div class="cup-ceremony-line">${us.map(player=>shirt(homeKit,player,0)).join('')}</div></div><div class="cup-ceremony-side right"><b>${escapeHTML(opponent.name)}</b><div class="cup-ceremony-line">${them.map(player=>shirt(awayKit,player,1)).join('')}</div></div></div><p class="cup-vs">Beide Teams stehen sich an der Mittellinie gegenüber.</p><button type="button" id="cup-final-start" class="primary">Finale starten ↗</button></section>`);
 $('#cup-final-start').onclick=()=>{$('#cup-ceremony').remove();$('#start').hidden=false;start()};
}
const v25ShowTacticsV41=showTactics;
showTactics=function(){v25ShowTacticsV41();if(!v41CupActive())return;v41CupAdvance();const game=v41CupGameForUser();if(!game)return;$('#heading').textContent=`Dein Plan fürs Pokal-${v41Labels[activeSave.cup.stage]}.`;$('#subtitle').textContent=`${activeSave.club} gegen ${activeOpponent().name}`;$('#start').innerHTML=`Pokalspiel starten <span>↗</span>`;v41RenderOrder();v41FinalCeremony()};
