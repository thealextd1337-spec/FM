...
function v55AllSkillsHTML(player){
 return `<div class="v55-roster-skills" role="list" aria-label="Alle Fähigkeiten">${v55SkillEntries(player).map(({label,value})=>`<span role="listitem" aria-label="${label}: ${v55SkillBandNames[v55SkillBand(value)]}"><span style="color:${v55SkillColor(value)}">${label}</span></span>`).join('')}</div>`;
}
